...
const Hero = ({ data }: HeroProps) => {
  ...
  const { heading, subheading, ctaText, infoText, backgroundImage } = data;

  // Build image URL with Sanity's image URL builder
  ...
  const imageWidth = backgroundImage.asset?.metadata?.dimensions?.width || 1400;
  const imageHeight =
    backgroundImage.asset?.metadata?.dimensions?.height || 900;
